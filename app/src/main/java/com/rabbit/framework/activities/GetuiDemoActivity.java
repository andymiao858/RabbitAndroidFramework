...
import com.rabbit.framework.broadcastreceiver.RabbitPushReceiver;
import com.rabbit.framework.handler.IEventBusHandler;
import com.rabbit.framework.managers.UserManager;
import com.rabbit.framework.models.event.MessageEvent;
import com.rabbit.framework.network.GetuiSdkHttpPost;
 ...
 * @author miaohd
 */
public class GetuiDemoActivity extends BaseEventActivity implements View.OnClickListener, IEventBusHandler {


	/**
	 * 第三方应用Master Secret，修改为正确的值
	 */
	private static final String MASTERSECRET = "a8L312Rg2f5k2UuhDZfvV4";
...
//		PushManager.getInstance().initialize(this.getApplicationContext());
		 ...
		 */
		if (RabbitPushReceiver.payloadData != null) {
			tLogView.append(RabbitPushReceiver.payloadData);
		}
		String cid = UserManager.getInstance().getUser().getClientId();
		if (Validator.isNotEmpty(cid)) {
			tView.setText(cid);
		}
	...
	public void onDestroy() {
		...
		RabbitPushReceiver.payloadData.delete(0, RabbitPushReceiver.payloadData.length());
		super.onDestroy();
	}
	...
	public void onEvent(MessageEvent messageEvent) {
		RLog.d("GetuiDemoActivity recv data : " + messageEvent.getData());
		if (MessageEvent.EventType.CLIENTID.equals(messageEvent.getEventType())){
			tView.setText((String)messageEvent.getData());
		}
	}
					...
					.setPositiveButton(android.R.string.ok, new DialogInterface.OnClickListener() {
						...
						public void onClick(DialogInterface dialog, int which) {
							...
							if (alias != null && alias.length() > 0) {
								...
								PushManager.getInstance().unBindAlias(GetuiDemoActivity.this, alias, true);
